...
module Bisim = struct
  ...
  let run file_or_pol1 file_or_pol2 json stdin containment () =
    let pol1 = parse_pol ~json (if stdin then `String file_or_pol1 else `File file_or_pol1) in
    let pol_tmp = parse_pol ~json (if stdin then `String file_or_pol2 else `File file_or_pol2) in

    let open Netkat.Syntax in
    let pol2 = if containment then Union (pol1, pol_tmp) else pol_tmp in

    let open Netkat.Global_compiler.Automaton in
    ...
    let dead = (Netkat.Fdd.FDD.drop, Netkat.Fdd.FDD.drop) in
    let dead1 = a1.nextState in
    let dead2 = a2.nextState in
    Hashtbl.add_exn a1.states ~key:dead1 ~data:dead;
    Hashtbl.add_exn a2.states ~key:dead2 ~data:dead;

    (* initialize worklist *)
    let pks = Netkat.Domain.(merge (of_automaton a1) (of_automaton a2) |> representative_pks) in
    let worklist = List.map pks ~f:(fun pk -> (a1.source, a2.source, pk)) in

    let rec member (s1y, s2y, pky) x =
      match x with 
      | [] -> false
      | (s1x, s2x, pkx) :: tl -> if Netkat.Packet.T.compare pkx pky = 0 && s1x = s1y && s2y = s2x then true else member (s1y, s2y, pky) tl
    in
    (* main loop *)
    let rec equiv worklist accum =
      match worklist with
      | [] ->
        true
      | (s1, s2, pk) :: worklist ->
        if member (s1, s2, pk) accum then equiv worklist accum else
        let e1, d1 = Hashtbl.find_exn a1.states s1 in
        let e2, d2 = Hashtbl.find_exn a2.states s2 in
        if not (Set.equal (Netkat.Packet.eval_e_fdd e1 pk) (Netkat.Packet.eval_e_fdd e2 pk)) then false else
        let worklist = 
          Map.fold2
            ~f:(fun ~key:pk -> fun ~data:d -> fun worklist -> match d with
              | `Both (s1, s2) -> (s1, s2, pk)::worklist
              | `Left s1 -> (s1, dead2, pk)::worklist
              | `Right s2 -> (dead1, s2, pk)::worklist
            )
            (Netkat.Packet.eval_d_fdd d1 pk: int64 Core.Map.M(Netkat.Packet.T).t)
            (Netkat.Packet.eval_d_fdd d2 pk: int64 Core.Map.M(Netkat.Packet.T).t) 
            ~init:(worklist:(int64 * int64 * Netkat.Packet.t) list) 
        in
        equiv (worklist:(int64 * int64 * Netkat.Packet.t) list) ((s1, s2, pk) :: accum) 
    in
    let open Core.Printf in
    if equiv (worklist:(int64 * int64 * Netkat.Packet.t) list) [] then printf("true\n") else printf("false\n");
